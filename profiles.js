// ============================================
//       FIEREN-BOT - SISTEMA DE PERFILES
// ============================================
...
// Volume persistente en Railway y que los perfiles no se borren en cada redeploy.
...
function xpBar(current, needed, length = 10) {
  // BUG FIX: si `current` es negativo o mayor que `needed` (puede pasar por
  // datos de XP inconsistentes, ej. tras algún ajuste manual o un cálculo
  // previo desalineado), "filled" salía fuera de [0, length] y
  // "length - filled" quedaba negativo, lo que rompía String.repeat() con
  // "RangeError: Invalid count value". Se acota filled a ese rango.
  let filled = Math.round((current / needed) * length);
...
async function cmdProfile(sock, msg, sender, from, mentionedJid, gacha, economy) {
  ...
  const text =
    ...
    `☆ Género ⋆ ${genreEmoji} ${user.genre || "No establecido"}\n` +
    `${bdayInfo}` +
