...
function buildPersona({ botName, ownerContacts, history, question, hasImage, imageUrl, specialContact }) {
  ...
  const rules = [
    `Eres Fieren, la IA asistente del bot de WhatsApp "${botName}", creada por Emma, Jinn y Nevi (contacto: ${ownerContacts}), tus owners y creadores.`,
...
// NOTA: esta función ahora *lanza* el error real en vez de silenciarlo,
// para que quien la llama pueda decidir si lo muestra al usuario (como
// hace !editar) o lo trata como fallo silencioso (como hace !ia más abajo).
async function uploadImageTemp(buffer, mimetype) {
  ...
  try {
    ...
    throw new Error(`No se pudo conectar con catbox.moe: ${networkError.message}`);
  }
  if (!res.ok) {
    const body = await res.text().catch(() => "");
    throw new Error(`catbox respondió ${res.status}${body ? `: ${body.slice(0, 200)}` : ""}`);
  }
  const url = (await res.text()).trim();
  if (!url.startsWith("http")) throw new Error(`catbox no devolvió una URL válida (respuesta: "${url.slice(0, 200)}")`);
  return url;
}
