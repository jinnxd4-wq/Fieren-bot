...
//   Los archivos en /plugins se cargan como comandos extra.
//   Cada plugin exporta: { name: "cmd" | ["cmd","alias"], ownerOnly, run(ctx) }
// ════════════════════════════════════════════════════════════
  ...
  for (const file of files) {
    ...
    try {
      ...
      for (const n of names) {
        if (n) pluginCommands.set(String(n).toLowerCase(), plugin);
      }
    } catch (error) {
      console.error(`❌ Error cargando plugin ${file}:`, error?.message || error);
    }
  }
...
// Modo privado global (!modeowner): si está activo, el bot ignora a
// cualquiera que no sea owner, en todos los sockets a la vez.
global.modoPrivadoOwner = global.modoPrivadoOwner ?? false;
const execPromise = util.promisify(exec);

// ════════════════════════════════════════════════════════════
//   SISTEMA DE MENSAJES (portado de new_bot/lib/console.js)
//   Logger de consola con caja decorada para cada mensaje entrante/saliente.
// ════════════════════════════════════════════════════════════

function _bytesToHuman(size) {
  if (!size) return "0 B";
  const units = ["B", "KB", "MB", "GB", "TB"];
  const power = Math.floor(Math.log(size) / Math.log(1000));
  const safePower = Math.min(Math.max(power, 0), units.length - 1);
  const value = size / (1000 ** safePower);
  return `${value.toFixed(1)} ${units[safePower]}`;
}

function _getMessageType(message = {}) {
  if (!message || typeof message !== "object") return "Desconocido";
  const firstKey = Object.keys(message)[0];
  if (!firstKey) return "Desconocido";
  return firstKey.replace(/message$/i, "").replace(/^./, (v) => v.toUpperCase());
}

function _estimateMessageSize(msg) {
  try {
    return Buffer.byteLength(JSON.stringify(msg?.message || {}), "utf8");
  } catch {
    return 0;
  }
}

function _formatTimeFromMessage(msg) {
  const raw = msg?.messageTimestamp;
  if (!raw) return new Date().toTimeString().split(" ")[0];
  const secs = typeof raw === "number" ? raw : (raw.low || Number(raw) || Math.floor(Date.now() / 1000));
  return new Date(secs * 1000).toTimeString().split(" ")[0];
}
...
// direction: "in" | "out"
function logMessage({ sock, msg, text, sender, chat, direction = "in", isSubBot = false }) {
  const me = sock.user?.id?.split(":")[0] || "desconocido";
  const botName = sock.user?.name || "Sin nombre";
  const filesize = _estimateMessageSize(msg);
  const mtype = _getMessageType(msg.message);
  const bodyText = String(text ?? _extractText(msg.message) ?? "").trim();
  const dirText = direction === "out" ? "SALIENTE" : "ENTRANTE";
  const botTag = isSubBot ? " (sub bot)" : "";

  console.log(chalk.cyanBright(`╔══════════════ ⋆⋅❆彡[ꜰɪᴇʀᴇɴ-ʙᴏᴛ${botTag}]彡❆⋅⋆ ════════════════•| ❅ |•`));
  console.log(chalk.blueBright("║"));
  console.log(`${chalk.whiteBright("╠═")} 🔧  ${chalk.cyanBright("Bot:")} ${chalk.blueBright(me)} ~${chalk.gray(botName)}`);
  console.log(`${chalk.blueBright("╠═")} 🔁  ${chalk.cyanBright("Direccion:")} ${chalk.whiteBright(dirText)}`);
  console.log(`${chalk.cyanBright("╠═")} 🕒  ${chalk.whiteBright("Fecha:")} ${chalk.blueBright(_formatTimeFromMessage(msg))}`);
  console.log(`${chalk.gray("╠═")} 📌  ${chalk.cyanBright("Tipo de evento:")} ${chalk.whiteBright(msg.messageStubType ? String(msg.messageStubType) : "Ninguno")}`);
  console.log(`${chalk.whiteBright("╠═")} 🧱  ${chalk.blueBright("Peso del mensaje:")} ${chalk.cyanBright(`${filesize} B`)} [${chalk.gray(_bytesToHuman(filesize))}]`);
  console.log(`${chalk.cyanBright("╠═")} 📤  ${chalk.blueBright("Remitente:")} ${chalk.whiteBright(sender)}`);
  console.log(`${chalk.blueBright("╠═")} 💬  ${chalk.cyanBright("Chat:")} ${chalk.whiteBright(chat)}`);
  console.log(`${chalk.gray("╠═")} 📨  ${chalk.cyanBright("Tipo de mensaje:")} ${chalk.blueBright(mtype)}`);
  if (bodyText) {
    console.log(`${chalk.whiteBright("╠═")} 📝  ${chalk.cyanBright("Texto:")} ${chalk.gray(bodyText)}`);
  }
  console.log(chalk.cyanBright("╚════════ ⋆⋅·͙⁺˚❅•̩̩͙❆•̩̩͙˚⁺‧͙⁺˚❅•̩̩͙ ᴄᴏɴꜱᴏʟᴇ •̩̩͙❆⁺‧͙⁺˚❅•̩̩͙❆•̩̩͙˚⁺⋅⋆ ════════════•| ❅ |•"));
}
...
try {
  ...
  // require() solo devuelve la ruta esperada del binario; no garantiza que
  // el archivo exista (si el postinstall de ffmpeg-static no pudo
  // descargarlo -por falta de internet durante el npm install, node_modules
  // copiado de otra plataforma, etc.- la ruta existe como string pero el
  // archivo no está en disco y spawn() falla con ENOENT).
  ffmpegPath = staticPath && fs.existsSync(staticPath) ? staticPath : "ffmpeg";
  if (staticPath && ffmpegPath === "ffmpeg") {
    logError("[ffmpeg-static] El binario no existe en disco, usando 'ffmpeg' del sistema como fallback:", staticPath);
  }
} catch {
  ffmpegPath = "ffmpeg";
}
...
function applyFierenBranding(sock) {
  ...
  // Set de IDs de mensajes que el propio bot mandó, para poder ignorarlos de
  // forma confiable cuando vuelven a llegar por "messages.upsert" (WhatsApp
  // marca fromMe=true tanto en las respuestas del bot como en los mensajes
  // reales que el dueño del número vinculado escribe con su teléfono — con
  // solo "fromMe" no se pueden distinguir). Se limpian solos a los 5 min.
  sock._fierenSentIds = new Set();
  sock.sendMessage = async (jid, content, options) => {
    ...
    try {
      logMessage({
        sock,
        msg: { message: content, messageTimestamp: Math.floor(Date.now() / 1000) },
        text: content?.text || content?.caption || "",
        sender: sock.user?.id?.split(":")[0] || "desconocido",
        chat: jid,
        direction: "out",
        isSubBot: isSubBotSock(sock),
      });
    } catch {}
    const id = result?.key?.id;
    if (id) {
      sock._fierenSentIds.add(id);
      setTimeout(() => sock._fierenSentIds.delete(id), 5 * 60 * 1000);
    }
    return result;
  };
  return sock;
...
// NOTA: igual que con los demás endpoints de alyacore, no logré confirmar el
// nombre exacto del parámetro de búsqueda ("q") ni la forma de la respuesta
// exitosa (mi herramienta de pruebas no reenvía el query string a este
// dominio), así que la extracción abajo prueba varios nombres de campo
// posibles. Se usa solo como respaldo si yt-search (la librería, que no
// depende de ninguna API externa) falla o no encuentra nada — si el nombre
// del campo está mal, simplemente no habrá resultados del respaldo y seguirá
// funcionando yt-search normal.
async function searchYoutubeAlyacore(query) {
  const { json, raw } = await fetchAlyacoreTextSearch("/search/yt", query);
  const list = json?.result || json?.results || json?.data || json?.items || [];
  if (!Array.isArray(list) || !list.length) throw new Error(`alyacore (search/yt) sin resultados: ${raw.slice(0, 300)}`);
  const videos = list
    .map(v => ({
      type: "video",
      videoId: v?.videoId || v?.id || null,
      title: v?.title || v?.titulo || "Sin título",
      url: v?.url || v?.link || (v?.videoId ? `https://youtu.be/${v.videoId}` : null),
      timestamp: v?.duration || v?.duracion || v?.timestamp || "Desconocido",
      ago: v?.ago || v?.publicado || "Desconocido",
      views: v?.views || v?.vistas || 0,
      thumbnail: v?.thumbnail || v?.miniatura || v?.image || null,
      image: v?.thumbnail || v?.miniatura || v?.image || null,
      author: { name: v?.author || v?.canal || v?.channel || "Desconocido" },
    }))
    .filter(v => v.url);
  if (!videos.length) throw new Error(`alyacore (search/yt) sin resultados usables: ${raw.slice(0, 300)}`);
  return videos;
}

// Intenta yt-search primero; si falla o no encuentra nada, usa alyacore.
async function ytSearchWithFallback(query) {
  try {
    const r = await yts(query);
    const all = r?.all?.length ? r.all : (r?.videos?.length ? r.videos : []);
    if (all.length) return { videos: r.videos || all.filter(v => v.type === "video"), all };
  } catch (e) {
    logError("[ytSearch] yt-search falló, probando alyacore:", e?.message || e);
  }
  const videos = await searchYoutubeAlyacore(query);
...
async function getVideoInfo(input, videoId) {
  ...
  const search = await ytSearchWithFallback(input);
...
// ── apicausas v2 (ytmp3 / ytmp4) ────────────────────────────────────────────
// Reemplaza a AetherAPI para las descargas de YouTube.
// GET https://rest.apicausas.xyz/api/v2/descargas/youtube?apikey=...&url=...&type=audio|video&quality=720 (quality solo aplica a video)
const APICAUSAS_V2_YT_BASE = "https://rest.apicausas.xyz/api/v2/descargas/youtube";

async function fetchApicausasV2Youtube(url, type, quality) {
  const qs = new URLSearchParams({ apikey: settings.apis.apicausas, url, type });
  if (type === "video" && quality) qs.set("quality", quality);
  const apiUrl = `${APICAUSAS_V2_YT_BASE}?${qs.toString()}`;
  const res = await fetch(apiUrl, { headers: { accept: "application/json", "user-agent": "Mozilla/5.0" } });
  const text = await res.text();
  if (!res.ok) throw new Error(`apicausas v2 HTTP ${res.status}: ${text.slice(0, 300)}`);
  let json;
  try { json = JSON.parse(text); } catch { throw new Error(`Respuesta no-JSON de apicausas v2: ${text.slice(0, 300)}`); }
  return { json, raw: text };
}

// ── ytmp3 (comando play) — vía apicausas v2 ─────────────────────────────────
async function getAudioFromApis(url) {
  try {
    const { json, raw } = await fetchApicausasV2Youtube(url, "audio");
    const root = (json?.data && typeof json.data === "object") ? json.data : json;
    const link = root?.download?.url || root?.url || root?.download_url || root?.downloadUrl || root?.link || root?.result?.url || root?.dl_url;
    if (!link) throw new Error(`apicausas v2 no devolvió un enlace de descarga: ${raw.slice(0, 300)}`);
    const name = root?.title || root?.filename || root?.titulo || "audio.mp3";
    return { url: link, name, api: "apicausas v2" };
  } catch (e) {
    logError(`[ytmp3] apicausas v2 falló:`, e?.message || e);
    throw new Error(`No se pudo obtener el audio: ${e.message}`);
  }
}

// ── ytmp4 — vía apicausas v2 ─────────────────────────────────────────────────
async function getVideoFromAetherApi(url, quality = "720") {
  const { json, raw } = await fetchApicausasV2Youtube(url, "video", quality);
  const root = (json?.data && typeof json.data === "object") ? json.data : json;
  const downloadUrl = root?.download?.url || root?.url || root?.download_url || root?.downloadUrl || root?.link || root?.result?.url || root?.dl_url;
  if (!downloadUrl) {
    throw new Error(`apicausas v2 no devolvió un enlace de descarga: ${raw.slice(0, 300)}`);
  }

  // Mismo formato que usaba el código anterior para no tocar el resto de handleYtMp4
  return {
    status: true,
...
// Descarga un archivo a Buffer con headers de navegador. Baileys descarga las
// URLs que se le pasan directo con axios y sin Referer/User-Agent, y varios
// hosts de descarga (ej. CDNs de audio de YouTube) devuelven 403 sin esos
// headers. Descargar acá primero y mandar el Buffer evita ese problema.
async function downloadBufferWithHeaders(url, referer = "https://www.youtube.com/") {
  const res = await fetch(url, {
    headers: {
  ...
  if (!res.ok) throw new Error(`Descarga falló con HTTP ${res.status}`);
  const buffer = Buffer.from(await res.arrayBuffer());
  if (!buffer.length) throw new Error("El archivo descargado está vacío");
  return buffer;
}
...
// ── Helper genérico para api.alyacore.xyz ───────────────────────────────────────
// NOTA: no logré confirmar el nombre exacto de los parámetros de query contra
// esta API (mi entorno de pruebas no reenvía bien el query string), así que
// los nombres usados abajo ("q", "url", "apikey") son la convención más común
// para este tipo de API, pero pueden estar mal. Si algo falla, el mensaje de
// error del bot va a incluir la respuesta cruda de la API — con eso se ajusta
// el nombre del parámetro correcto en una sola línea.
async function fetchAlyacore(path, params) {
  const qs = new URLSearchParams({ apikey: settings.apis.alyacore, ...params });
  const apiUrl = `https://api.alyacore.xyz${path}?${qs.toString()}`;
  const res = await fetch(apiUrl, { headers: { accept: "application/json", "user-agent": "Mozilla/5.0" } });
  const text = await res.text();
  if (!res.ok) throw new Error(`alyacore (${path}) HTTP ${res.status}: ${text.slice(0, 300)}`);
  let json;
  try { json = JSON.parse(text); } catch { throw new Error(`Respuesta no-JSON de alyacore (${path}): ${text.slice(0, 300)}`); }
  if (json?.status === false) throw new Error(`alyacore (${path}) → ${json?.message || "sin detalle"}: ${text.slice(0, 300)}`);
  return { json, raw: text };
}

// Variante POST (JSON body) del helper de arriba. Algunos endpoints de alyacore
// devuelven "falta el parámetro X" sin importar qué se mande en el query string
// de un GET — eso suele indicar que solo leen el body de un POST. Se usa como
// último recurso si todos los intentos por GET fallan.
async function fetchAlyacorePost(path, params) {
  const apiUrl = `https://api.alyacore.xyz${path}`;
  const res = await fetch(apiUrl, {
    method: "POST",
    headers: { "content-type": "application/json", accept: "application/json", "user-agent": "Mozilla/5.0" },
    body: JSON.stringify({ apikey: settings.apis.alyacore, ...params }),
  });
  const text = await res.text();
  if (!res.ok) throw new Error(`alyacore (${path}) POST HTTP ${res.status}: ${text.slice(0, 300)}`);
  let json;
  try { json = JSON.parse(text); } catch { throw new Error(`Respuesta no-JSON de alyacore (${path}) POST: ${text.slice(0, 300)}`); }
  if (json?.status === false) throw new Error(`alyacore (${path}) POST → ${json?.message || "sin detalle"}: ${text.slice(0, 300)}`);
  return { json, raw: text };
}

// Variante para endpoints que reciben un solo texto/búsqueda como parámetro.
// Como no se pudo confirmar el nombre real del parámetro contra esta API,
// prueba varios nombres comunes en orden hasta que uno funcione (status !== false)
// y se queda con ese para el resto del proceso. Si todos los intentos por GET
// fallan, prueba las mismas variantes por POST (body JSON) antes de rendirse,
// por si el endpoint solo lee el body. Si todo falla, lanza el error del
// último intento (con la respuesta cruda incluida).
const ALYACORE_TEXT_PARAM_CANDIDATES = ["prompt", "text", "q", "query", "texto", "search"];
// Mismo problema que con los demás endpoints de alyacore: no se pudo
// confirmar el nombre real del parámetro de imagen contra la API real desde
// acá (no hay forma de probar peticiones POST en este entorno). "image" es
// el candidato más probable porque el mensaje de error de /ai/gpt-editor
// habla literalmente de "un prompt", calcando el nombre de parámetro más
// común para este tipo de API. Si falla, el error trae la respuesta cruda
// de la API para ajustar el nombre correcto en una sola línea.
const ALYACORE_IMAGE_PARAM_CANDIDATES = ["image", "img", "imagen", "url", "foto"];
async function fetchAlyacoreTextSearch(path, value, extraParams = {}) {
  let lastError = null;
  for (const paramName of ALYACORE_TEXT_PARAM_CANDIDATES) {
    try {
      return await fetchAlyacore(path, { [paramName]: value, ...extraParams });
    } catch (e) {
      lastError = e;
    }
  }
  for (const paramName of ALYACORE_TEXT_PARAM_CANDIDATES) {
    try {
      return await fetchAlyacorePost(path, { [paramName]: value, ...extraParams });
    } catch (e) {
      lastError = e;
    }
  }
...
// ── ai/chatgpt via alyacore ──────────────────────────────────────────────────
// NOTA: igual que el resto de endpoints de alyacore, no confirmé el nombre
// real del parámetro (probé "text" porque el mensaje de error dice "ingresa
// un texto", pero podría ser "q"/"prompt") ni la forma de la respuesta
// exitosa. Si falla, el error trae la respuesta cruda para ajustar rápido.
async function askAlyacoreChatGPT(text) {
  const { json, raw } = await fetchAlyacoreTextSearch("/ai/chatgpt", text);
  const root = (json?.data && typeof json.data === "object") ? json.data : json;
  const answer = root?.respuesta || root?.response || root?.answer || root?.message || root?.result || root?.text;
  if (!answer) throw new Error(`alyacore (chatgpt) no devolvió una respuesta: ${raw.slice(0, 300)}`);
  return String(answer);
}

// ── ai/gpt-editor via alyacore (edición de imágenes con prompt de texto) ─────
// Envuelve la instrucción del usuario en un prompt base para resultados más
// consistentes: que la IA respete lo que se le pide sin romper el resto de
// la imagen (identidad de las personas, fondo, estilo) ni agregar cosas de
// más por su cuenta.
function buildGptEditorPrompt(userInstruction) {
  return (
    `Edita esta imagen siguiendo estrictamente esta instrucción: "${userInstruction}". ` +
    `Mantené el resto de la imagen sin cambios (mismas personas, mismo fondo, mismo estilo) ` +
    `salvo lo que la instrucción pida modificar explícitamente. No agregues texto, marcas de agua ` +
    `ni elementos que no se hayan pedido. Priorizá una edición realista y de alta calidad.`
  );
}

// Igual que los demás endpoints de alyacore: no se pudo confirmar contra la
// API real el nombre exacto de los parámetros de imagen/prompt ni el nombre
// del campo de la imagen resultante en la respuesta, así que se prueban las
// combinaciones más probables por POST (la API respondió con el mismo error
// de "falta el prompt" incluso con el query string bien armado en un GET, lo
// que indica que solo lee el body de un POST — mismo patrón que el resto de
// endpoints de este archivo). Si todas las combinaciones fallan, se lanza el
// error del último intento con la respuesta cruda incluida, para poder
// ajustar el nombre correcto rápido con eso a la vista.
async function alyacoreGptEditor(buffer, prompt) {
  const imageB64 = buffer.toString("base64");
  let lastError = null;
  for (const imgParam of ALYACORE_IMAGE_PARAM_CANDIDATES) {
    for (const promptParam of ALYACORE_TEXT_PARAM_CANDIDATES) {
      try {
        const { json, raw } = await fetchAlyacorePost("/ai/gpt-editor", {
          [imgParam]: imageB64,
          [promptParam]: prompt,
        });
        const root = (json?.data && typeof json.data === "object") ? json.data : json;
        const imageOut =
          root?.image || root?.result || root?.url || root?.b64_json || root?.output || root?.foto;
        if (imageOut) return String(imageOut);
        lastError = new Error(`alyacore (gpt-editor) no devolvió una imagen usable: ${raw.slice(0, 300)}`);
      } catch (e) {
        lastError = e;
      }
    }
...
// ── ai/copilot via alyacore ───────────────────────────────────────────────────
// Mismo patrón que askAlyacoreChatGPT(): usa el helper genérico que prueba
// varios nombres de parámetro y GET/POST hasta que uno funcione, y reutiliza
// la misma apikey de alyacore ya configurada en settings.apis.alyacore.
async function askAlyacoreCopilot(text) {
  const { json, raw } = await fetchAlyacoreTextSearch("/ai/copilot", text);
  const root = (json?.data && typeof json.data === "object") ? json.data : json;
  const answer = root?.result || root?.response || root?.answer || root?.text || root?.message;
  if (!answer) throw new Error(`alyacore (copilot) no devolvió una respuesta: ${raw.slice(0, 300)}`);
  return String(answer);
}

// ── ai/gpt-editor via alyacore ───────────────────────────────────────────────
// NOTA: igual que las otras funciones de alyacore, no confirmé el nombre real
// de los parámetros (probablemente "url" + "prompt", pero puede variar) ni la
// forma de la respuesta exitosa. Si falla, el error incluye la respuesta cruda
// para poder ajustar rápido.
async function getGptEditedImage(imageUrl, prompt) {
  const { json, raw } = await fetchAlyacore("/ai/gpt-editor", { url: imageUrl, prompt });
  const root = (json?.data && typeof json.data === "object") ? json.data : json;
  const resultUrl = root?.url || root?.result || root?.image || root?.output || root?.link || root?.result_url;
  if (!resultUrl) throw new Error(`alyacore (gpt-editor) no devolvió una imagen editada: ${raw.slice(0, 300)}`);
  return resultUrl;
}
...
async function handleYtMp3(sock, from, msg, query) {
  ...
  try {
    ...
    let audio;
    try {
      audio = await getAudioFromApis(url);
    } catch (e1) {
      logError("[ytmp3] alyacore falló:", e1?.message || e1);
      await reactToMessage(sock, msg, "❌");
    ...
    let audioPayload = { url: audio.url };
    try {
      audioPayload = await downloadBufferWithHeaders(audio.url);
    } catch (dlErr) {
      logError("[ytmp3] descarga manual del audio falló, se intenta con la URL directa:", dlErr?.message || dlErr);
    }
...
async function handleYtMp4(sock, from, msg, query) {
  ...
  try {
    ...
    let data;
    try {
      data = await getVideoFromAetherApi(url);
    } catch (apicausasErr) {
      logError("[ytmp4] apicausas v2 falló:", apicausasErr);
      await reactToMessage(sock, msg, "❌");
      await sock.sendMessage(from, { text: `❌ No se pudo obtener el video.\n_${apicausasErr.message}_` });
      return;
    }
...
async function handleYtSearch(sock, from, msg, query) {
  ...
  try {
    const results = await ytSearchWithFallback(query);
...
// ════════════════════════════════════════════════════════════
//   PINTEREST — vía apicausas (rest.apicausas.xyz/buscadores/pinterest)
// ════════════════════════════════════════════════════════════

async function handlePinterest(sock, from, msg, query) {
  if (!query) {
    await sock.sendMessage(from, {
      text: `Usa: *${settings.prefix}pin <búsqueda>*\nEjemplo: *${settings.prefix}pin aesthetic room*`,
    });
    return;
  }
  ...
  try {
    const results = await fetchPinterest(query);
    ...
    for (let i = 0; i < selected.length; i++) {
      try {
        ...
        const caption =
          ...
          (r.title && r.title !== "Pinterest Pin" ? `📌 *Título ›* ${r.title}\n` : "") +
          (r.author_username ? `👤 *Autor ›* ${r.author_username}\n` : "") +
          `🔗 *Enlace ›* ${r.link}`;
...
// JID especial de WhatsApp para los estados (historias). Se usa junto con
// el comando !statusview para marcar automáticamente los estados como vistos.
...
// Determina si el `sock` que está procesando el mensaje es un sub-bot
// (vinculado con !code/!qr) o el bot principal, comparando su número
// contra el registro `subBots`. Se usa para que los logs y comandos
// (!menu, etc.) muestren el tipo de socket correcto, en vez de asumir
// siempre que es el bot principal.
function isSubBotSock(sock) {
  const num = bareNumber(getBotId(sock));
  if (Object.prototype.hasOwnProperty.call(subBots, num)) return true;
  return Object.values(subBots).some((entry) => entry?.realNumber === num);
}
...
function isSocketOwner(sock, sender, senderIsOwner, msg) {
  ...
  const botNum = bareNumber(botId);
  // BUG FIX: tras reconectar, sock.user.id (y por tanto botId/getBotId) a
  // veces queda como un JID @lid en vez del número real de teléfono del
  // sub-bot, mientras que el dueño que lo vinculó sigue mandando mensajes
  // desde su número normal (@s.whatsapp.net). Sin esto, isSocketOwner()
  // nunca coincidía y el propio dueño del sub-bot quedaba bloqueado de
  // TODOS los comandos de configuración (setbanner, setname, setprefix...).
  // Se busca el registro del sub-bot (por clave o por realNumber ya
  // guardado en connection.update) y se agrega su número real a la lista.
  const subEntry =
    subBots[botNum] ||
    Object.values(subBots).find(
      (entry) => entry?.realNumber === botNum || bareNumber(entry?.sock?.user?.id) === botNum
    );
  const list = [botId, config.owner, subEntry?.realNumber]
    .filter(Boolean)
    .map((jid) => bareNumber(jid));
  const candidates = [
    sender,
...
// Resuelve, para comandos de configuración (setname/setbanner/etc.), a qué
// bot apuntar: si quien manda el comando es owner GLOBAL y menciona/cita a
// otro bot registrado (principal o cualquier sub-bot), apunta a ESE bot en
// vez del socket actual — así se puede renombrar/cambiar el banner de un
// sub-bot sin tener que estar metido en su propia sesión. Si no hay mención,
// o quien manda no es owner global, se usa el comportamiento de siempre
// (el socket actual, sujeto a isSocketOwner()).
// Devuelve { targetBotId, isRemote } o null si la mención no corresponde a
// ningún bot conocido (para poder avisarle al usuario).
function resolveConfigTarget(sock, sender, senderIsOwner, msg) {
  const mentionedRaw = senderIsOwner ? (getMentionedJid(msg) || getQuotedParticipant(msg)) : null;
  if (!mentionedRaw) return { targetBotId: getBotId(sock), isRemote: false };

  const targetNum = bareNumber(mentionedRaw);
  const mainBotNum = bareNumber(getBotId(sock));
  const isMainBot = targetNum === mainBotNum;
  const isKnownSub =
    Object.prototype.hasOwnProperty.call(subBots, targetNum) ||
    Object.values(subBots).some((entry) => entry?.realNumber === targetNum);

  if (!isMainBot && !isKnownSub) return null;
  return { targetBotId: `${targetNum}@s.whatsapp.net`, isRemote: true };
}
...
// NOTA: antes esto encolaba los procesos de ffmpeg para que nunca corriera
// más de uno a la vez (cada ffmpeg es un proceso aparte, fuera del heap de
// Node, y suma memoria extra). Se quitó esa cola a pedido explícito: ahora
// cada llamada a runFfmpeg() lanza su propio proceso ffmpeg en paralelo sin
// ninguna limitación. Con poca RAM disponible en el contenedor, varios
// comandos pesados (sticker/video) al mismo tiempo pueden hacer que el bot
...
async function handleAnimeReaction(sock, msg, sender, from, commandKey) {
  ...
  // Igual que !setprimary: acepta tanto @mención como responder (citar) al
  // mensaje de la persona objetivo, en vez de exigir siempre una @mención.
  const mention = getMentionedJid(msg) || getQuotedParticipant(msg);
  if (config.needMention && !mention) {
    await sock.sendMessage(from, { text: `Debes mencionar (o responder) a alguien para usar *${settings.prefix}${commandKey}*.` });
    return true;
  }
  ...
  if (config.source === "apicausas") {
    ...
    // BUG FIX: nekos.best no tenía ningún respaldo — si fallaba (caído,
    // rate limit, endpoint sin resultado), el comando mandaba solo el
    // texto sin video/gif (ej. !punch). Ahora, igual que "apicausas",
    // cae a apicausas y luego a waifu.pics antes de rendirse.
    if (!mediaUrl) mediaUrl = await fetchApicausasAnime(commandKey);
    if (!mediaUrl && WAIFUPICS_MAP[commandKey]) mediaUrl = await fetchWaifuPics(WAIFUPICS_MAP[commandKey]);
...
const subSubBotWatchdogs = {}; // { [phoneNumber]: intervalId } — watchdog anti-zombie por sub-bot
...
// ── Sub-bot "principal" por grupo ──
// Cuando el bot principal y uno o más sub-bots están en el mismo grupo, sin
// esto TODOS responderían al mismo comando. !setprimary (por grupo) elige
// cuál de ellos es el único que debe procesar comandos en ese grupo.
// Se guarda en db.js bajo el JID del grupo (clave "primaryBot" = número bare).
function getAllBotNumbers() {
  const nums = new Set(Object.keys(subBots));
  for (const entry of Object.values(subBots)) {
    if (entry?.realNumber) nums.add(entry.realNumber);
  }
  if (mainSock?.user?.id) nums.add(bareNumber(mainSock.user.id));
  return nums;
}

// BUG FIX: WhatsApp también le asigna un @lid al propio bot dentro de cada
// grupo (igual que a cualquier participante) — igual que ya se maneja en
// isBotAdmin() con [sock.user?.id, sock.user?.lid]. getAllBotNumbers() solo
// conocía el número real, así que si alguien mencionaba o citaba al bot
// principal (o a un sub-bot) usando su @lid en vez de su número real,
// !setprimary nunca lo reconocía. Este mapa traduce CUALQUIER identidad
// conocida de un bot (número real o @lid) a su número real canónico.
function getBotIdentityMap() {
  const map = new Map();
  const add = (sockRef, realNumberOverride) => {
    const real = realNumberOverride || bareNumber(sockRef?.user?.id);
    if (!real) return;
    map.set(real, real);
    if (sockRef?.user?.id) map.set(bareNumber(sockRef.user.id), real);
    if (sockRef?.user?.lid) map.set(bareNumber(sockRef.user.lid), real);
  };
  add(mainSock);
  for (const entry of Object.values(subBots)) {
    add(entry?.sock, entry?.realNumber);
  }
  return map;
}

function getGroupPrimaryBot(groupJid) {
  return db.getSettings(groupJid)?.primaryBot || null;
}

function setGroupPrimaryBot(groupJid, phoneNumber) {
  db.setSettings(groupJid, "primaryBot", phoneNumber || null);
}
...
// Sube el banner a cdn.dix.lat y devuelve la URL pública.
// Usa el fetch/FormData/Blob nativos de Node (18+) vía globalThis, en vez del
// shim de node-fetch definido más arriba (const fetch = ...), para no
// depender de paquetes extra (axios, formdata-node, file-type) que además
// suelen distribuirse solo en ESM y no se pueden usar con require().
async function uploadBannerToCDN(buffer, filename, mimetype) {
  const form = new globalThis.FormData();
  const blob = new globalThis.Blob([buffer], { type: mimetype || "image/jpeg" });
  form.append("file", blob, filename);

  const res = await globalThis.fetch("https://cdn.dix.lat/upload", {
    method: "POST",
    body: form,
  });
  const text = await res.text();
  if (!res.ok) throw new Error(`cdn.dix.lat HTTP ${res.status}: ${text.slice(0, 300)}`);

  let data;
  try { data = JSON.parse(text); } catch { throw new Error(`Respuesta no válida de cdn.dix.lat: ${text.slice(0, 300)}`); }

  const url = data?.data?.url;
  if (!data?.status || !url) throw new Error(`cdn.dix.lat rechazó la subida: ${text.slice(0, 300)}`);
  return url;
}
...
async function startSubBot(phoneNumber, ownerSock, ownerChatId, ownerMsg, isCode = true) {
  ...
  const subSock = makeWASocket({
    ...
    // Dejamos pasar status@broadcast (para poder marcar estados como vistos
    // con !statusview) pero seguimos ignorando el resto de broadcasts.
    shouldIgnoreJid: (jid) => jid.endsWith("@broadcast") && jid !== STATUS_JID,
  ...
  // BUG FIX: antes se esperaba al evento "qr" dentro de connection.update
  // para recién ahí pedir el pairing code. Ese es el patrón usado para
  // mostrar un QR, pero para pedir un CÓDIGO no hace falta esperarlo — de
  // hecho en varias versiones/redes ese "qr" nunca llega cuando se va a usar
  // código en vez de QR, y el bot se quedaba esperando para siempre sin
  // enviar nada. El patrón oficial de Baileys pide el código apenas se crea
  // el socket (con un pequeño margen para que termine de inicializarse),
  // sin depender de ningún evento.
  const requestPairingCodeNow = async () => {
    ...
    try {
      ...
      log.info(`Sub-bot ${phoneNumber}: código de vinculación generado: ${code}`);
      // BUG FIX: antes el código se mandaba en un mensaje "interactivo" con
      // botón de copiar (nativeFlowMessage), un tipo de mensaje frágil que
      // en ciertas versiones/clientes de WhatsApp falla al construirse o al
      // entregarse SIN lanzar un error visible (su propio try/catch interno
      // ya intentaba un fallback a texto plano, pero evidentemente algo en
      // esa cadena se estaba perdiendo el mensaje por completo). Se manda
      // directo como texto normal: 100% confiable, y el código se puede
      // copiar igual con solo mantener presionado el mensaje.
      // El código va en un mensaje APARTE del texto informativo, para que al
      // mantener presionado solo se seleccione/copie el código, sin mezclarse
      // con el resto del texto.
      // El código se manda solo, en un único mensaje de texto plano — sin
      // bordes, emojis ni mensaje informativo previo — para que se pueda
      // copiar directo sin nada más alrededor.
      const codeMsg = await liveOwnerSock.sendMessage(liveOwnerChatId, { text: formatted }, { quoted: liveOwnerMsg });
      setTimeout(() => {
        liveOwnerSock.sendMessage(liveOwnerChatId, { delete: codeMsg.key }).catch(() => {});
      }, 60_000);
    } catch (error) {
      logError(`Sub-bot ${phoneNumber}: error generando el código:`, error);
      const owner = subBots[phoneNumber] || { ownerSock, ownerChatId, ownerMsg };
      delete subBots[phoneNumber];
      try { subSock.end(undefined); } catch {}
      try {
        await owner.ownerSock.sendMessage(owner.ownerChatId, {
          text: `❌ No se pudo generar el código para el sub-bot.\n> ${error?.message || "Error desconocido"}`,
        }, { quoted: owner.ownerMsg });
      } catch {}
    }
  };

  // DIAGNÓSTICO: si en 20s no se pidió el código/qr ni la conexión se cerró,
  // algo se quedó atascado en el handshake con WhatsApp (antes esto fallaba
  // en silencio total: sin código, sin error, para siempre). Avisamos al
  // owner y dejamos rastro en consola para poder diagnosticarlo.
  const stallTimeout = setTimeout(() => {
    if (!pairingRequested && subBots[phoneNumber]?.sock === subSock) {
      log.warn(`Sub-bot ${phoneNumber}: sigue en "connecting" tras 20s, no se pudo generar el código/qr.`);
  ...
  if (isCode) {
    // Pequeño margen para que el socket termine de inicializar el handshake
    // antes de pedir el código, tal como recomienda la documentación de Baileys.
    setTimeout(() => { requestPairingCodeNow().catch(() => {}); }, 3000);
  }
  ...
  subSock.ev.on("connection.update", async ({ connection, qr, lastDisconnect }) => {
    ...
    if (qr && !state.creds.registered && !pairingRequested) {
      if (isCode) {
        // Ya se pidió (o se está pidiendo) el código apenas se creó el
        // socket — ver requestPairingCodeNow() más arriba. No hacemos nada
        // más aquí para evitar pedirlo dos veces.
      } else {
        pairingRequested = true;
        clearTimeout(stallTimeout);
        try {
    ...
    if (connection === "open") {
      clearTimeout(stallTimeout);
      // BUG FIX: antes se mandaba "vinculado y conectado correctamente" en
      // CADA apertura de conexión, incluidas las reconexiones automáticas
      // (que pueden pasar muy seguido si la conexión es inestable). Eso
      // llenaba el chat del owner con el mismo aviso cada 30-60 min. Ahora
      // solo se notifica la primera vez que este socket queda vinculado; las
      // reconexiones posteriores solo quedan en el log de consola.
      const yaHabiaNotificado = subBots[phoneNumber]?.hasNotifiedConnected;
      if (subBots[phoneNumber]) {
        subBots[phoneNumber].connected = true;
        subBots[phoneNumber].hasNotifiedConnected = true;
        // BUG FIX: el número usado para pedir el código (normalizePhone, con
        // ajustes regionales como el "9" de Argentina o el "1" de México)
        // puede no coincidir exactamente con el número real que WhatsApp
        // termina confirmando para la cuenta ya conectada. Si no coinciden,
        // !setprimary y las menciones nunca reconocían a este sub-bot porque
        // comparaban contra un número "casi correcto" pero distinto. Se
        // guarda aquí el número real para que getAllBotNumbers()/
        // isSubBotSock() lo tengan en cuenta, sin tocar la clave del
        // registro (que sigue igual para no romper la carpeta de sesión).
        subBots[phoneNumber].realNumber = bareNumber(subSock.user?.id) || phoneNumber;
      }
      subReconexion = 0;
      log.success(`Sub-bot ${phoneNumber} conectado${yaHabiaNotificado ? " (reconexión, sin avisar de nuevo)" : ""}.`);
      try { db.setSettings(`${phoneNumber}@s.whatsapp.net`, "type", "Sub"); } catch {}

      // ── Watchdog de conexión zombie (sub-bots) ──────────────────────────
      // Mismo problema que puede sufrir el bot principal: WhatsApp deja de
      // mandar eventos pero "close" nunca llega, y el sub-bot se queda
      // "conectado" en apariencia sin procesar nada hasta un reinicio
      // manual. Antes solo el bot principal tenía este chequeo; ahora cada
      // sub-bot también prueba la conexión cada 2 min y se fuerza a cerrar
      // (lo que sí dispara la reconexión normal de más abajo) si no responde.
      if (subSubBotWatchdogs[phoneNumber]) clearInterval(subSubBotWatchdogs[phoneNumber]);
      // BUG FIX: jitter en el intervalo (2 min ± hasta 20s) para que, aunque
      // varios sub-bots hayan conectado casi al mismo tiempo, sus sondas no
      // caigan siempre en el mismo instante y generen ráfagas simultáneas
      // contra WhatsApp desde la misma IP.
      const watchdogIntervalMs = 2 * 60 * 1000 + Math.floor(Math.random() * 20_000);
      subSubBotWatchdogs[phoneNumber] = setInterval(async () => {
        if (subBots[phoneNumber]?.sock !== subSock) {
          clearInterval(subSubBotWatchdogs[phoneNumber]);
          delete subSubBotWatchdogs[phoneNumber];
          return;
        }
        try {
          const probeNumber = (subSock.user?.id || "").split(":")[0].split("@")[0];
          await Promise.race([
            subSock.onWhatsApp(probeNumber),
            new Promise((_, reject) => setTimeout(() => reject(new Error("timeout")), 15_000)),
          ]);
        } catch (err) {
          log.warn(`Watchdog: sub-bot ${phoneNumber} no respondió (${err?.message || err}) — forzando reconexión.`);
          clearInterval(subSubBotWatchdogs[phoneNumber]);
          delete subSubBotWatchdogs[phoneNumber];
          try { subSock.end(new Error("watchdog: sin respuesta")); } catch {}
        }
      }, watchdogIntervalMs);

      if (!yaHabiaNotificado) {
        try {
          await liveOwnerSock.sendMessage(liveOwnerChatId, {
    ...
    if (connection === "close") {
      clearTimeout(stallTimeout);
      if (subSubBotWatchdogs[phoneNumber]) {
        clearInterval(subSubBotWatchdogs[phoneNumber]);
        delete subSubBotWatchdogs[phoneNumber];
      }
...
async function loadExistingSubBots(ownerSock) {
  ...
  for (const entry of entries) {
    ...
    // BUG FIX: antes todos los sub-bots arrancaban casi en el mismo
    // instante (un simple await entre cada uno, sin pausa real), así que
    // sus watchdogs (cada 2 min desde que abren) quedaban sincronizados y
    // disparaban ráfagas simultáneas de sondas contra WhatsApp desde la
    // misma IP cada 2 minutos. Con las horas, eso puede hacer que WhatsApp
    // empiece a limitar esas conexiones en silencio (siguen "conectadas"
    // pero dejan de recibir mensajes). Se agrega una pausa entre cada
    // arranque para desincronizarlos.
...
// Pide el número por consola cada vez que hace falta vincular.
// Sustituye al antiguo número fijo (DEFAULT_PAIRING_NUMBER) que siempre
// era el mismo.
function askPhoneNumber() {
  ...
  return (async () => {
    let answer = "";
    while (!normalizePhone(answer)) {
      answer = await question(chalk.cyanBright("  📱 Ingresa el número de WhatsApp a vincular (con código de país, sin +): "));
      if (!normalizePhone(answer)) console.log(chalk.red("  Número inválido, intenta de nuevo."));
    }
    rl.close();
...
async function chooseLinkMethod() {
  ...
  // Vinculación siempre por código (nunca QR).
  opcion = "2";
...
async function startBot() {
  ...
  // A veces Baileys deja el socket "medio abierto": WhatsApp deja de mandar
  // eventos pero el evento connection.update con "close" nunca llega, así
  // que la reconexión automática de más abajo no se dispara y el bot se
  // queda mudo hasta un reinicio manual. Cada 2 min se prueba una acción
  // liviana contra el socket (actualizar presencia); si no responde a
  // tiempo, se lo fuerza a cerrar para que SÍ dispare "close" y entre a la
  // reconexión normal (los sub-bots no suelen sufrir esto porque se
  // reconectan/renuevan con más frecuencia).
  const WATCHDOG_INTERVAL_MS = 2 * 60 * 1000;
  const WATCHDOG_TIMEOUT_MS  = 15 * 1000;
  const watchdogInterval = setInterval(async () => {
    if (!botReady || mainSock !== sock) return;
    try {
      // OJO: sendPresenceUpdate() NO sirve como sonda — es un stanza "fire and
      // forget", la promesa se resuelve al escribirlo en el socket local sin
      // esperar nada del servidor, así que nunca detecta una conexión zombie
      // (que es justo el caso que este watchdog debía cubrir). onWhatsApp()
      // sí hace una consulta (usync) que espera respuesta real del servidor.
      const probeNumber = (sock.user?.id || "").split(":")[0].split("@")[0];
      await Promise.race([
        sock.onWhatsApp(probeNumber),
        new Promise((_, reject) => setTimeout(() => reject(new Error("timeout")), WATCHDOG_TIMEOUT_MS)),
      ]);
    } catch (err) {
      log.warn(`Watchdog: el bot principal no respondió (${err?.message || err}) — forzando reconexión.`);
      clearInterval(watchdogInterval);
      try { sock.end(new Error("watchdog: sin respuesta")); } catch {}
    }
  ...
  // BUG FIX: mismo problema que en los sub-bots — pedir el código recién al
  // recibir el evento "qr" podía dejarlo esperando para siempre si ese
  // evento no llegaba. Lo pedimos apenas se crea el socket (patrón oficial
  // de Baileys), con un pequeño margen para que termine de inicializarse.
  if (!state.creds.registered && phoneNumber) {
    setTimeout(async () => {
      if (pairingCodeRequested || state.creds.registered) return;
...
async function handleMessages(sock, { messages, type }) {
    ...
    // ── Ver estados automáticamente (toggle por socket: !statusview) ──
    // Los estados llegan con remoteJid = "status@broadcast" y no son chats
    // reales, así que se procesan aparte y no siguen al resto del flujo
    // (comandos, IA, anti-link, etc.).
    if (from === STATUS_JID) {
      if (!msg.key.fromMe) {
        try {
          const statusCfg = db.getSettings(getBotId(sock)) || {};
          if (statusCfg.statusview) {
            await sock.readMessages([
              { remoteJid: STATUS_JID, id: msg.key.id, participant: msg.key.participant },
            ]);
          }
        } catch (error) {
          logError("Error marcando estado como visto:", error);
        }
      }
      return;
    }
    ...
    if (msg.key.fromMe) {
      // Ignoramos los mensajes que el propio bot mandó (lo sabemos con
      // certeza por el ID, ver applyFierenBranding) para no crear loops.
      // Cualquier OTRO mensaje fromMe=true es porque el dueño del número
      // vinculado lo escribió de verdad con su teléfono (WhatsApp marca
      // fromMe=true en todo lo que sale de esa cuenta, sea el chat privado
      // consigo mismo, un grupo, o un chat con otra persona) — a partir de
      // ahora se procesa exactamente igual que el mensaje de cualquier otro
      // usuario, no solo si "parece" un comando.
      if (sock._fierenSentIds?.has(msg.key.id)) return;
    }
    ...
    logMessage({ sock, msg, text: body, sender, chat: from, direction: "in", isSubBot: isSubBotSock(sock) });
    ...
    // ── Bot primario del grupo (!setprimary) ──
    // Si este grupo tiene un bot marcado como primario y no es este socket,
    // solo cede el turno si ese bot primario sigue siendo válido (parte de
    // la flota); si no, se abre el paso para que cualquier bot responda
    // (evita que el grupo se quede sin respuesta si el primario se
    // desvinculó por completo).
    //
    // BUG FIX: antes se exigía que getAllBotNumbers().has(groupPrimary) Y
    // que sock.groupMetadata() (llamada a la API en CADA mensaje) mostrara
    // al primario como participante. !setprimary ya no valida el número
    // resuelto contra getAllBotNumbers() (se sacó esa validación por líos
    // con @lid), así que ese "has()" casi nunca daba true y el filtro nunca
    // se activaba — todos los bots seguían respondiendo. Ahora se usa
    // getBotIdentityMap(), que ya resuelve @lid ↔ número real de forma
    // consistente, y no depende de una llamada a la API por mensaje.
    if (isGroup) {
      const rawGroupPrimary = getGroupPrimaryBot(from);
      if (rawGroupPrimary) {
        const identityMap = getBotIdentityMap();
        const ownNumber = identityMap.get(bareNumber(sock.user?.id)) || bareNumber(sock.user?.id);
        // BUG FIX: si !setprimary guardó un @lid que en su momento no se pudo
        // resolver a número real (quedó tal cual, como dígitos de lid), este
        // valor nunca coincidía con knownBotNumbers (que solo contiene
        // números reales) y el gate nunca se activaba para NINGÚN bot —
        // todos seguían respondiendo. Ahora se resuelve rawGroupPrimary a
        // través del identityMap (que sí mapea lid → real) antes de comparar.
        const groupPrimary = identityMap.get(rawGroupPrimary) || rawGroupPrimary;
        if (groupPrimary !== ownNumber) {
          const knownBotNumbers = new Set(identityMap.values());
          if (knownBotNumbers.has(groupPrimary)) return;
        }
      }
    }
    ...
    // ── Modo Privado global (!modeowner on/off) ──
    // Ignora a todo el que no sea owner, salvo que esté ejecutando
    // justamente el comando para desactivarlo.
    if (global.modoPrivadoOwner && !senderIsOwner && command !== "modeowner") return;
    ...
    switch (command) {
      ...
      case "ayuda":
        await reactToMessage(sock, msg, "📋");
        {
          ...
          const menuCaption =
              ...
              `┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n` +
              `☆ Autor ⋆ Jinn y Emma\n` +
              `☆ Bot ⋆ ${isSubBotSock(sock) ? "Sub-Bot" : "Principal"}\n` +
              ...
              `☆ *${p}setprimary » ${p}setbot* + <mención>\n   ↳ Elige qué bot (principal o sub-bot) responde comandos en este grupo cuando hay varios presentes.\n` +
              `☆ *${p}resetprimary » ${p}resetbot*\n   ↳ Quita el bot primario para que todos vuelvan a responder.\n` +
              `\n` +
              ...
              `☆ *${p}chatgpt » ${p}gpt* + <pregunta / responde a un mensaje>\n   ↳ Habla con ChatGPT (respuesta única, sin memoria de conversación).\n` +
              `☆ *${p}copilot* + <pregunta / responde a un mensaje>\n   ↳ Habla con Copilot (respuesta única, sin memoria de conversación).\n` +
              `☆ *${p}gptedit* + <instrucciones>, respondiendo o enviando una imagen\n   ↳ Edita una imagen con IA según tus instrucciones (ej: *${p}gptedit ponle un sombrero de mago*).\n` +
              ...
              `☆ *${p}statusview* + <on/off>\n   ↳ Activa/desactiva que este socket marque automáticamente los estados como vistos.\n` +
              `\n` +
              ...
              `\n` +
              `┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n` +
              `♡ Creado con mucho amor por Jinn, los amo ❤️`;
      ...
      case "subbots": {
        ...
        if (totalSubs) {
          report += `\n*Lista de sub-bots:*\n`;
          report += nums.map((n) => {
            const entry = subBots[n];
            const realTag = entry.realNumber && entry.realNumber !== n ? ` — número real: ${entry.realNumber}` : "";
            return `• wa.me/${n} — ${entry.connected ? "🟢 conectado" : "🟡 vinculando..."}${realTag}`;
          }).join("\n");
        } else {
      ...
      case "setprimary":
      case "setbot": {
        // Cuando el bot principal y uno o más sub-bots están juntos en el mismo
        // grupo, sin esto todos responderían al mismo comando. Este comando
        // (por grupo) elige cuál de esos números debe ser el único que procese
        // comandos aquí — ver el gate correspondiente en handleMessages().
        //
        // NOTA: a diferencia de una versión anterior, este comando YA NO exige
        // que el número resuelto coincida con una lista interna de "bots
        // conocidos" (getAllBotNumbers()). Esa validación fallaba una y otra
        // vez porque WhatsApp no siempre expone el número real (PN) de un
        // @lid de forma consistente entre mención, cita y group metadata.
        // Ahora, igual que en Ellen-Joe-Bot: se resuelve el @lid contra
        // `participants` si aplica, y se confía directamente en el resultado
        // (mención, mensaje citado, o número escrito a mano).
        if (!isGroup) { await sock.sendMessage(from, { text: "⚠️ Este comando solo puede usarse en grupos." }, { quoted: msg }); break; }

        const senderIsAdmin = await isAdmin(sock, from, sender, msg);
        if (!senderIsAdmin && !senderIsOwner) {
          await sock.sendMessage(from, { text: "❌ Solo los admins del grupo pueden usar este comando." }, { quoted: msg });
          break;
        }

        let who = getMentionedJid(msg) || getQuotedParticipant(msg);
        if (!who && args[0]) {
        ...
        if (!who) {
          await sock.sendMessage(from, {
            text: `⚠️ Debes mencionar, responder, o escribir el número del bot que deseas establecer como primario.\nEj: *${settings.prefix}setprimary @521234567890*`,
          }, { quoted: msg });
          break;
        }

        // Si el JID viene como @lid, resolverlo al número real (PN). Primero
        // se prueba contra getBotIdentityMap() (mapea el @lid de CADA bot de
        // la flota a su número real de forma directa, sin depender de que
        // WhatsApp lo exponga bien en el groupMetadata); si no aparece ahí
        // (ej. es un @lid de un participante que no es ninguno de nuestros
        // bots), se cae al groupMetadata como respaldo.
        let botJid = who;
        if (who.endsWith("@lid")) {
          const lidDigits = bareNumber(who);
          const knownReal = getBotIdentityMap().get(lidDigits);
          if (knownReal) {
            botJid = `${knownReal}@s.whatsapp.net`;
          } else {
            const metadata = await sock.groupMetadata(from).catch(() => null);
            const pInfo = metadata?.participants?.find((p) => p.lid === who || p.jid === who);
            if (pInfo?.id) botJid = pInfo.id;
          }
        }

        const targetNumber = bareNumber(botJid);
        if (getGroupPrimaryBot(from) === targetNumber) {
          await sock.sendMessage(from, { text: `✨ @${targetNumber} ya es el bot primario de este grupo.`, mentions: [botJid] }, { quoted: msg });
          break;
        }

        setGroupPrimaryBot(from, targetNumber);
        await sock.sendMessage(from, {
          text: `⭐ Se estableció a @${targetNumber} como bot primario de este grupo.\n> A partir de ahora, solo ese número procesará comandos aquí (el resto se queda callado).\n> Usa *${settings.prefix}resetprimary* para que todos los bots vuelvan a responder.`,
          mentions: [botJid],
        }, { quoted: msg });
        break;
      }
      ...
      case "resetprimario": {
        ...
        const senderIsAdmin2 = await isAdmin(sock, from, sender, msg);
        if (!senderIsAdmin2 && !senderIsOwner) {
          await sock.sendMessage(from, { text: "❌ Solo los admins del grupo pueden usar este comando." }, { quoted: msg });
          break;
        }

        if (!getGroupPrimaryBot(from)) {
          await sock.sendMessage(from, { text: "《✧》No hay ningún bot primario establecido en este grupo." }, { quoted: msg });
          break;
        }

        setGroupPrimaryBot(from, null);
        await sock.sendMessage(from, {
          text: "✐ ¡Listo! Se restableció la configuración.\n> A partir de ahora, todos los bots volverán a responder en este grupo.",
        }, { quoted: msg });
        break;
      }
      ...
      case "estado":
        ...
        await sock.sendMessage(from, {
          text:
            `*Estado del bot*\n\n` +
            `Nombre: ${getBotDisplayName(sock)}\n` +
            `Tipo de socket: ${isSubBotSock(sock) ? "Sub-Bot" : "Principal"}\n` +
            `Tu rol: ${senderIsOwner ? "Owner" : "Usuario"}\n` +
            `Prefijo: ${usedPrefix || getBotPrefixes(sock)[0] || settings.prefix}\n` +
      ...
      case "sockets": {
        ...
        for (const num of Object.keys(subBots)) {
          ...
          const realNum = entry?.realNumber || num;
          const jid = `${realNum}@s.whatsapp.net`;
          ...
          lines.Sub.push(`- [Sub ${entry?.connected ? "🟢" : "🟡"} *${subName}*] › @${realNum}`);
        }
      ...
      case "statusview": {
        const botId = getBotId(sock);
        const cfg = db.getSettings(botId) || {};
        const isOn = Boolean(cfg.statusview);
        const opt = (args[0] || "").toLowerCase();
        if (opt === "on" || opt === "enable") {
          if (isOn) { await sock.sendMessage(from, { text: "☆彡 El visto automático de estados ya estaba activado en este socket." }); break; }
          db.setSettings(botId, "statusview", true);
          await sock.sendMessage(from, { text: "☆彡 Activado ✅ — este socket marcará automáticamente los estados de sus contactos como vistos." });
        } else if (opt === "off" || opt === "disable") {
          if (!isOn) { await sock.sendMessage(from, { text: "☆彡 El visto automático de estados ya estaba desactivado en este socket." }); break; }
          db.setSettings(botId, "statusview", false);
          await sock.sendMessage(from, { text: "☆彡 Desactivado ❌ — este socket ya no marcará los estados como vistos." });
        } else {
          await sock.sendMessage(from, { text: `Usa: *${usedPrefix}statusview on* ó *${usedPrefix}statusview off*\n> Estado actual en este socket: ${isOn ? "Activado ✅" : "Desactivado ❌"}` });
        }
        break;
      }
      ...
      case "setbotname":
      case "setname": {
        const target = resolveConfigTarget(sock, sender, senderIsOwner, msg);
        if (!target) { await sock.sendMessage(from, { text: "❀ Ese número no está registrado como bot principal ni como sub-bot." }); break; }
        if (!target.isRemote && !isSocketOwner(sock, sender, senderIsOwner, msg)) { await sock.sendMessage(from, { text: "☆彡 Este comando solo puede ser ejecutado por un owner del socket." }); break; }
        // Si se apuntó a otro bot por mención, esa parte del texto queda como
        // "@1234567890" dentro de args — se limpia antes de parsear el nombre.
        const value = args.join(" ").replace(/@\d+/g, "").trim();
        ...
        db.setSettings(target.targetBotId, "namebot", short.trim());
        db.setSettings(target.targetBotId, "botname", long.trim());
        const who = target.isRemote ? `@${target.targetBotId.split("@")[0]}` : "este socket";
        await sock.sendMessage(from, {
          text: `✿ El nombre de ${who} ha sido actualizado!\n\n❒ Nombre corto: *${short.trim()}*\n❒ Nombre largo: *${long.trim()}*`,
          mentions: target.isRemote ? [target.targetBotId] : [],
        });
        break;
      }
      ...
      case "setbanner": {
        const configTarget = resolveConfigTarget(sock, sender, senderIsOwner, msg);
        if (!configTarget) { await sock.sendMessage(from, { text: "❀ Ese número no está registrado como bot principal ni como sub-bot." }); break; }
        if (!configTarget.isRemote && !isSocketOwner(sock, sender, senderIsOwner, msg)) { await sock.sendMessage(from, { text: "☆彡 Este comando solo puede ser ejecutado por un owner del socket." }); break; }
        const target = quotedImageMessage ? { message: { imageMessage: quotedImageMessage } } : (isImage ? msg : null);
        if (!target) { await sock.sendMessage(from, { text: "❀ Responde/envía una imagen citándola con este comando." }); break; }
        try {
          const buffer = await downloadMediaMessage(target, "buffer", {}, {});
          if (!buffer || !buffer.length) throw new Error("Buffer vacío");
          const imgMsg = target.message?.imageMessage;
          const mimetype = imgMsg?.mimetype || "image/jpeg";
          const ext = mimetype.split("/")[1]?.split(";")[0] || "jpg";
          const filename = `banner_${configTarget.targetBotId.split("@")[0]}_${Date.now()}.${ext}`;

          const bannerUrl = await uploadBannerToCDN(buffer, filename, mimetype);
          db.setSettings(configTarget.targetBotId, "banner", bannerUrl);
          const who = configTarget.isRemote ? `@${configTarget.targetBotId.split("@")[0]}` : "este socket";
          await sock.sendMessage(from, {
            text: `✿ Banner actualizado para ${who}.\n🔗 *URL:* ${bannerUrl}\n> Nota: WhatsApp no expone una "portada" de perfil vía API; esta imagen se guarda (ahora en cdn.dix.lat) para usarla en tus propias tarjetas de perfil/menú.`,
            mentions: configTarget.isRemote ? [configTarget.targetBotId] : [],
          });
        } catch (error) {
...
async function fetchBratImage(text) {
  try {
    ...
    return Buffer.from(await res.arrayBuffer());
      ...
      case "chatgpt":
      case "gpt": {
        const gptQuestion = (args.join(" ") || getQuotedText(msg)).trim();
        if (!gptQuestion) {
          await sock.sendMessage(from, {
            text: `☆ Usa: *${settings.prefix}chatgpt <pregunta>* (o responde a un mensaje)\nEjemplo: *${settings.prefix}chatgpt ¿qué es una API?*`,
          });
          break;
        }
        await reactToMessage(sock, msg, "🤖");
        try {
          const answer = await askAlyacoreChatGPT(gptQuestion);
          await sock.sendMessage(from, { text: answer }, { quoted: msg });
          await reactToMessage(sock, msg, "✅");
        } catch (error) {
          logError("Error en chatgpt:", error);
          await reactToMessage(sock, msg, "❌");
          await sock.sendMessage(from, {
            text: `❌ No pude obtener respuesta de ChatGPT.\n_${error.message}_`,
          }, { quoted: msg });
        }
        break;
      }
      ...
      case "copilot": {
        // Adaptado del plugin copilot.js (formato de otro framework, con
        // import/export y un helper reply()) al estilo switch/case de este
        // bot. Se reutiliza fetchAlyacoreTextSearch()/settings.apis.alyacore
        // en vez de la apikey y el axios.get() hardcodeados en el original,
        // ya que el proyecto ya tiene esa infraestructura para /ai/*.
        // NOTA: se quitó el alias "ai" del plugin original porque ya existe
        // el comando !ai/!ia (runAiQuery, con memoria e imagen) — usarlo acá
        // también habría chocado con ese comando.
        const copilotText = (args.join(" ") || getQuotedText(msg)).trim();
        if (!copilotText) {
          await sock.sendMessage(from, {
            text: `☆ Escribe tu pregunta o mensaje para Copilot.\nEjemplo: *${settings.prefix}copilot ¿cómo está el clima hoy?*`,
          }, { quoted: msg });
          break;
        }

        const copilotSent = await sock.sendMessage(from, { text: "> *Copilot está procesando tu petición...*" }, { quoted: msg });
        try {
          const copilotAnswer = await askAlyacoreCopilot(copilotText);
          await sock.sendMessage(from, { text: copilotAnswer, edit: copilotSent.key });
        } catch (error) {
          logError("Error en copilot:", error);
          await sock.sendMessage(from, { text: `❌ No pude obtener respuesta de Copilot.\n_${error.message}_`, edit: copilotSent.key });
        }
        break;
      }
      ...
      case "gptedit":
      case "iaedit": {
        const editPrompt = args.join(" ").trim();
        if (!editPrompt) {
          await sock.sendMessage(from, {
            text: `☆ Usa: *${settings.prefix}gptedit <instrucciones>*, respondiendo o enviando una imagen.\nEjemplo: *${settings.prefix}gptedit ponle un sombrero de mago*`,
          });
          break;
        }

        let editTarget, editMimetype;
        if (isImage) { editTarget = msg; editMimetype = directMessage.imageMessage.mimetype; }
        else if (quotedImage) { editTarget = { key: msg.key, message: { imageMessage: quotedImageMessage } }; editMimetype = quotedImageMessage.mimetype; }
        else {
          await sock.sendMessage(from, { text: `☆ Responde o envía una imagen junto con *${settings.prefix}gptedit <instrucciones>*.` });
          break;
        }

        await reactToMessage(sock, msg, "🎨");
        try {
          const editBuffer = await downloadMediaMessage(editTarget, "buffer", {}, {});
          const editExt = (editMimetype || "image/jpeg").split("/")[1]?.split(";")[0] || "jpg";
          const editFilename = `fierenbot_${Date.now()}.${editExt}`;
          const editForm = new FormData();
          editForm.append("files[]", new Blob([editBuffer], { type: editMimetype }), editFilename);
          const uploadRes = await globalThis.fetch("https://uguu.se/upload.php", { method: "POST", body: editForm });
          if (!uploadRes.ok) throw new Error(`uguu.se respondió con estado ${uploadRes.status}`);
          const uploadJson = await uploadRes.json();
          const tempImageUrl = uploadJson?.files?.[0]?.url;
          if (!tempImageUrl) throw new Error("No se pudo subir la imagen para editarla.");

          const resultUrl = await getGptEditedImage(tempImageUrl, editPrompt);
          await sock.sendMessage(from, {
            image: { url: resultUrl },
            caption: `🎨 *Editado con IA*\n📝 ${editPrompt}`,
          }, { quoted: msg });
          await reactToMessage(sock, msg, "✅");
        } catch (error) {
          logError("Error en editar (gpt-editor):", error);
          await reactToMessage(sock, msg, "❌");
          await sock.sendMessage(from, {
            text: `❌ No pude editar la imagen.\n_${error.message}_`,
          }, { quoted: msg });
        }
        break;
      }
...
// Detecta el tipo real de una imagen por sus primeros bytes (magic numbers),
// en vez de confiar solo en el mimetype que reporta WhatsApp — así un
// archivo mal etiquetado falla acá con un mensaje claro, en vez de fallar
// más adelante en la API externa con un error críptico.
// (Tomado de YukiBot-MD, que usa file-type; acá se hace a mano para no
...
async function vectorinkFetchWithRetry(buffer, { maxRetries = 3, timeoutMs = 120_000 } = {}) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      res = await fetch(API, {
        ...
        // MEJORA (de YukiBot-MD): sin esto, un vectorink.io colgado dejaba la
        // petición esperando para siempre y el comando !hd nunca respondía
        // nada (ni éxito ni error).
        signal: AbortSignal.timeout ? AbortSignal.timeout(timeoutMs) : undefined,
      });
    } catch (err) {
      lastError = err?.name === "TimeoutError" || err?.name === "AbortError"
        ? new Error(`vectorink.io no respondió en ${Math.round(timeoutMs / 1000)}s`)
        : err;
    ...
    // 429 = demasiadas peticiones, y los 5xx suelen ser fallos temporales del
    // lado del servidor. Ambos vale la pena reintentar con espera creciente.
    // Otros códigos (400, 403, etc.) son errores permanentes de la petición
    // en sí, así que ahí no tiene sentido reintentar.
    const retryable = res.status === 429 || res.status >= 500;
    lastError = new Error(`vectorink.io respondió con estado ${res.status}`);
    if (!retryable || attempt === maxRetries) throw lastError;

    // Si el servidor indica cuánto esperar (Retry-After), respetamos eso;
    // si no, backoff exponencial: 1.5s, 3s, 6s...
    const retryAfterHeader = Number(res.headers.get("retry-after"));
    const waitMs = Number.isFinite(retryAfterHeader) && retryAfterHeader > 0
      ? retryAfterHeader * 1000
      : 1500 * 2 ** attempt;
    log.warn(`vectorink.io: intento ${attempt + 1}/${maxRetries + 1} falló (${res.status}), reintentando en ${Math.round(waitMs / 1000)}s...`);
    await new Promise((r) => setTimeout(r, waitMs));
  }
  throw lastError;
}

// MEJORA (de YukiBot-MD): timeout propio para el ffmpeg de esta conversión,
// para no depender de runFfmpeg() global (que no tiene límite de tiempo) y
// evitar que un proceso colgado deje el comando !hd esperando para siempre.
function runFfmpegWithTimeout(args, timeoutMs = 60_000) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error("ffmpeg: tiempo de espera agotado"));
    }, timeoutMs);
  });
}

async function vectorinkEnhance(buffer) {
  const res = await vectorinkFetchWithRetry(buffer);
  const json = await res.json();
  let inner;
  ...
  // MEJORA (de YukiBot-MD): carpeta temporal del sistema (os.tmpdir()) en
  // vez de una carpeta dentro del proyecto, + nombre único por petición
  // (timestamp + random) para que dos !hd simultáneos nunca choquen entre sí.
  const tempDir = path.join(os.tmpdir(), "fieren_vectorink");
  if (!fs.existsSync(tempDir)) fs.mkdirSync(tempDir, { recursive: true });
  const baseName = `hd_${Date.now()}_${Math.random().toString(16).slice(2)}`;
  const inputPath = path.join(tempDir, `${baseName}.webp`);
  const outputPath = path.join(tempDir, `${baseName}.png`);
  fs.writeFileSync(inputPath, webpBuffer);
  try {
    await runFfmpegWithTimeout(["-y", "-i", inputPath, "-frames:v", "1", outputPath]);
    return fs.readFileSync(outputPath);
  } finally {
    try { fs.unlinkSync(inputPath); } catch {}
    try { fs.unlinkSync(outputPath); } catch {}
      ...
      case "remini": {
        try {
          ...
          if (!buffer || !Buffer.isBuffer(buffer) || buffer.length < 10) {
            await reactToMessage(sock, msg, "❌");
            await sock.sendMessage(from, { text: "☆彡 No se pudo descargar la imagen." });
            break;
          }
          // MEJORA (de YukiBot-MD): se revalida el tipo real por los bytes
          // del archivo ya descargado (no solo el mimetype que reportó
          // WhatsApp), para no gastar la llamada a vectorink.io con un
          // archivo que en realidad no es una imagen soportada.
          const realMime = sniffImageMime(buffer) || mimetype;
          if (!/^image\/(jpe?g|png|webp)$/i.test(realMime)) {
            await reactToMessage(sock, msg, "❌");
            await sock.sendMessage(from, { text: `☆彡 El formato *${realMime}* no es compatible.` });
            break;
          }
      ...
      // ── Edita una imagen a partir de instrucciones de texto (IA) ──
      case "edit":
      case "gptedit": {
        try {
          let targetMessage, mimetype;
          if (isImage) { targetMessage = msg; mimetype = directMessage.imageMessage.mimetype; }
            ...
            await sock.sendMessage(from, {
              text: `☆ Responde a una *imagen* con *${settings.prefix}edit <instrucciones>* (ej. "${settings.prefix}edit ponle un sombrero de mago").`,
            });
            break;
          }
          if (!/^image\/(jpe?g|png|webp)/i.test(mimetype || "")) {
          ...
          const prompt = (args.join(" ") || getQuotedText(msg)).trim();
          if (!prompt) {
            await sock.sendMessage(from, {
              text: `☆ Decime qué querés que le haga a la imagen. Ej: *${settings.prefix}edit ponle un sombrero de mago*`,
            });
            break;
          }
          await reactToMessage(sock, msg, "🪄");
          const buffer = await downloadMediaMessage(targetMessage, "buffer", {}, {});
          const resultado = await alyacoreGptEditor(buffer, buildGptEditorPrompt(prompt));
          const isUrl = /^https?:\/\//i.test(resultado);
          await sock.sendMessage(
            from,
            isUrl ? { image: { url: resultado }, caption: "🪄 *Edición completada*" }
                  : { image: Buffer.from(resultado, "base64"), caption: "🪄 *Edición completada*" },
            { quoted: msg }
          );
          await reactToMessage(sock, msg, "✅");
        } catch (error) {
          logError("Error en edit:", error);
          await reactToMessage(sock, msg, "❌");
          await sock.sendMessage(from, {
            text: `☆彡 No pude editar la imagen.\n\n🔧 *Detalle técnico:*\n${String(error.message).slice(0, 400)}`,
          });
        }
        break;
      }

      // ── Verifica librerías, herramientas y sintaxis de plugins ──
      case "check":
      case "checkbot":
      case "verificar": {
        if (!senderIsOwner) { await sock.sendMessage(from, { text: "Este comando es solo para owners." }); break; }
        ...
        try {
          ...
          for (const dep of deps) {
            try { require.resolve(dep); libsOk.push(dep); }
            catch { libsFail.push(dep); }
          }
        } catch (error) {
          libsFail.push(`package.json: ${error.message}`);
        }
        ...
        if (fs.existsSync(PLUGINS_DIR)) {
          for (const file of fs.readdirSync(PLUGINS_DIR).filter((f) => f.endsWith(".js"))) {
            try {
              await execPromise(`node --check "${path.join(PLUGINS_DIR, file)}"`);
              pluginsOk.push(file);
            } catch (error) {
              pluginsFail.push({ name: file, error: (error.stderr || error.message || "").split("\n")[0] });
            }
          }
        }
        ...
        for (const tool of tools) {
          try {
            const { stdout } = await execPromise(`${tool} -version`);
        ...
        const nodeModulesExiste = fs.existsSync(path.join(__dirname, "node_modules"));

        let text = `🔍 *Verificación del sistema*\n\n`;
        text += `🔧 *Herramientas*\n`;
        for (const [tool, status] of Object.entries(toolsStatus)) {
          text += `  ${status.ok ? "✅" : "❌"} ${tool}${status.ok ? `: ${status.version}` : ": no encontrado"}\n`;
        }
        text += `  ${nodeModulesExiste ? "✅" : "❌"} node_modules${nodeModulesExiste ? ": instalado" : ": falta, correr npm install"}\n\n`;

        text += `📦 *Librerías* (${libsOk.length} ok)\n`;
        if (libsFail.length) text += libsFail.map((l) => `  ❌ ${l}`).join("\n") + "\n";
        else text += `  ✅ Todas instaladas\n`;
        text += "\n";

        text += `🧩 *Plugins* (${pluginsOk.length} ok)\n`;
        if (pluginsFail.length) text += pluginsFail.map((p) => `  ❌ ${p.name}: ${p.error?.slice(0, 60)}`).join("\n") + "\n";
        else text += `  ✅ Sin errores de sintaxis\n`;
      ...
      // ── Activa/desactiva el modo privado global (solo owners globales) ──
      case "modeowner": {
        if (!senderIsOwner) { await sock.sendMessage(from, { text: "Este comando es solo para owners." }); break; }
        const opt = (args[0] || "").toLowerCase();
        if (opt === "on") {
          ...
          await reactToMessage(sock, msg, "🔒");
          await sock.sendMessage(from, { text: `🔒 *Modo privado activado.*\n\n${getBotDisplayName(sock)} ignorará a todos menos a los owners, en todos los sockets.` });
        } else if (opt === "off") {
          global.modoPrivadoOwner = false;
          await reactToMessage(sock, msg, "🔓");
          await sock.sendMessage(from, { text: "🔓 *Modo privado desactivado.* El bot vuelve a responder a todos." });
        } else {
          await sock.sendMessage(from, { text: `Uso: *${usedPrefix}modeowner on* ó *${usedPrefix}modeowner off*\nEstado actual: ${global.modoPrivadoOwner ? "Activado" : "Desactivado"}` });
        }
        break;
      }

      // ── Guarda código citado como un nuevo plugin en /plugins y lo recarga ──
      case "saveplugin": {
        if (!senderIsOwner) { await sock.sendMessage(from, { text: "Este comando es solo para owners." }); break; }
        const quoted = msg.message?.extendedTextMessage?.contextInfo?.quotedMessage;
        const codigo = quoted?.conversation || quoted?.extendedTextMessage?.text;
        if (!codigo) {
          await sock.sendMessage(from, { text: "⚠️ Debes responder al mensaje que contiene el código del plugin (CommonJS: module.exports = { name, ownerOnly, async run(ctx) {...} })." });
          break;
        }
        if (!args[0]) {
          await sock.sendMessage(from, { text: `Uso: responde al código y escribe *${usedPrefix}saveplugin nombre.js*` });
          break;
        }
        let fileName = args[0].replace(/\.\.\//g, "").replace(/\//g, "_");
        ...
        try {
          ...
          loadPlugins();
          await reactToMessage(sock, msg, "✅");
          await sock.sendMessage(from, { text: `✅ Plugin guardado y cargado: *plugins/${fileName}*` });
        } catch (error) {
          await reactToMessage(sock, msg, "❌");
          await sock.sendMessage(from, { text: `❌ Error guardando el plugin: ${error.message}` });
        }
        break;
      }

      default: {
        // Comandos cargados dinámicamente desde /plugins
        const plugin = pluginCommands.get(command);
        if (plugin) {
          if (plugin.ownerOnly && !senderIsOwner) {
            await sock.sendMessage(from, { text: "Este comando es solo para owners." });
            break;
          }
          try {
            await plugin.run({
              sock, msg, from, sender, args, text: args.join(" "),
              settings, economy, isOwner: senderIsOwner, usedPrefix,
              reply: (content) => sock.sendMessage(from, content, { quoted: msg }),
              react: (emoji) => reactToMessage(sock, msg, emoji),
            });
          } catch (error) {
            logError(`Error en plugin "${command}":`, error);
            await sock.sendMessage(from, { text: `❌ Error en el plugin: ${String(error?.message || error).slice(0, 400)}` });
          }
          break;
        }
